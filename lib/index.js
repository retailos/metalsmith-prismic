    ...
    return function(files, metalsmith, done){
        ...
        function generateCollectionFiles(fileName, collectionQuery, callback, ctx) {
            if (collectionQuery != null) {
                ...
                var stripSlashRegex = /^\//g

                each(file.prismic[collectionQuery].results, (result, cb) => {
                    var newFile = Object.assign({}, file, {
                            prismic: Object.assign({}, file.prismic, {
                                [collectionQuery]: {
                                    results: [result]
                                }
                            })
                    ...
                    ctx.path = fileName;
                    newFiles[(ctx.linkResolver(result) + fileSuffix).replace(stripSlashRegex, '')] = newFile;
                    cb();
                }, (err) => {
